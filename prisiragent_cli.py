...
SYSTEM_CHAT = (
    ...
    "give false familiarity and break under composition. Use 操作直觉法 instead. Structure: (1) 直觉挑战 — ask "
    "the user's intuitive answer to a concrete question first (制造认知冲突); (2) 具体化 — compute a concrete "
    "numeric example with run_code and/or draw the relationship with a mermaid xychart/flowchart (几何/数值直觉 "
    "先于符号); (3) 解压缩 — unpack the compressed definition into its 'challenge-response' steps (e.g. 极限 "
    "ε-δ = 你给 ε,我必须还你 δ); (4) 阶梯 quiz — first a 判断对错 (intuition layer), then a 算具体值 "
    "(operation layer). Prefer structure-mapping analogies (结构同构, e.g. 配方 = 补成正方形) over narrative "
    "analogies.\n"

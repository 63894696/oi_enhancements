"""prisir_mcp_bridge.py — Prisir AI 的 MCP client 桥(迁入自 mcp_client_bridge)

把本地起的第三方 MCP server 的工具并入 Prisir AI 工具体系。

架构要点:
- Prisiragent web/cli 是同步架构(ThreadingHTTPServer + threading),
  而官方 mcp python sdk 的 ClientSession 是 async(anyio)。
- 桥的做法:每个连接一个常驻 asyncio event loop 线程 + actor 长跑协程,
  所有 async MCP 调用经队列提交,包成同步接口。不碰同步主流程。

配置: $PRISIR_DATA_DIR/etc/mcp_servers.json (默认 ~/.local/share/prisir/etc/mcp_servers.json)
  [
    {
      "name": "filesystem",
      "command": "npx",
...
log = logging.getLogger("prisiragent.mcp_bridge")


def _data_root() -> Path:
    """数据目录:与 learning_progress 一致(PRISIR_DATA_DIR 可覆盖,默认 ~/.local/share/prisir)。"""
    return Path(os.environ.get("PRISIR_DATA_DIR") or (Path.home() / ".local" / "share" / "prisir"))


# 配置路径:mcp_servers.json 放 prisir 数据目录 etc/ 下
MCP_SERVERS_CONFIG = _data_root() / "etc" / "mcp_servers.json"
...
class MCPBridge:
    ...
    def server_status(self) -> dict:
        ...
        return {
            name: {
                "connected": conn._connected_evt.is_set() and not conn.last_error,

# prisiragent-shell — Prisiragent 本地对话壳(Electron)

不启用浏览器也能和 Prisiragent 对话。主进程 spawn 并看护 `prisiragent_web.py`
(127.0.0.1:18802,国画风聊天 UI,SQLite 持久化),壳把它包成桌面窗口。

定位见 `Documents/prisiragent-os-integration/prisirwork-foundation-integration-design.md` §5.1(F7)。
与已归档的 `securedm-shell`(Tauri)不同——本壳走 Electron(拍板),复用 prisiragent_web.py。

## 跑起来

```bash
cd prisiragent-shell
npm install        # 装 electron(首次)
npm start          # 起壳:自动拉起 prisiragent_web.py 并开窗
```

要求:本机 `python` 可跑 `prisiragent_web.py`(其依赖 fastlane/prisiragent_cli 已装好)。
...
- **全局热键**:默认 `Ctrl+Shift+O`(`PRISIRAGENT_SHELL_HOTKEY` 可改,旧名 `OIAGENT_SHELL_HOTKEY` 兼容)。
...
- `prisiragent_web` 只监听 127.0.0.1;壳加载的也是回环地址。外链一律交系统浏览器,
  壳内不导航出回环。
...
| 变量 | 缺省 | 说明 |
|---|---|---|
| `PRISIRAGENT_WEB_PORT` | `18802` | prisiragent_web 端口(旧名 `OIAGENT_WEB_PORT` 兼容) |
| `PRISIRAGENT_SHELL_HOTKEY` | `Ctrl+Shift+O` | 全局呼出热键(旧名 `OIAGENT_SHELL_HOTKEY` 兼容) |
| `PRISIRAGENT_PYTHON` | `python` | 起 web 服务的 Python(旧名 `OIAGENT_PYTHON` 兼容) |
...
python .tmp_stage.py            # 拷 assets + prisiragent-shell 到 installer/_staging/
...
2. 装完桌面有「Prisir AI」.lnk(图标=对话壳火苗版 `prisiragent-shell/icon.ico`)。
...
- 不要直接从 `dist/assets`、`dist/prisiragent-shell` 拷(`File /r "..\dist\assets"` + SetOutPath=$INSTDIR 会创建 `$INSTDIR\dist\` 残留),统一从 `installer/_staging/` 拷。

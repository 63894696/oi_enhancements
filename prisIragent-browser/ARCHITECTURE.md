...
下图展示两大簇的关系:**prisiragent-browser 浏览器本体**(智能体层 / 网络链路层 / 密信 / 内容柜)与 **prisirwork 能力门面**(本地常驻 daemon),以及壳、模型端点池、自建节点等外部角色。

```mermaid
graph TD
    User[用户] -->|NTP 当场输入 / 预签 A1 授权| Shell["壳 Prisir Shell<br/>(NTP WebUI · chrome://agent)"]

    subgraph Browser["prisiragent-browser 浏览器本体(Chromium src)"]
        direction TB
...
### 2.1 agent 操作层(prisiragent-browser/agent)
...
### 2.2 proxy 网络链路层(prisiragent-browser/proxy)
...
### 2.4 prisiragent 协作团队

- prisiragent 为项目开发协作智能体,运行于本地对话壳(Prisir Shell),受【项目宪法】硬性契约约束(Ed25519 唯一签名、凭证只走环境变量、本地优先、红线诚实标注)。

...
# v0.25 task queue 5 tool — Claude 写 task + OIagent 调度 + 状态管理
...
def build_server() -> Server:
    ...
    async def handle_list_tools() -> list[types.Tool]:
        return [
            ...
            # ────────────────────────────────────────
            # v0.25 Task Queue 5 tool(Claude 写 task,OIagent 调度)
            # 设计:Claude 只管记忆链接 + 写 task;OIagent 自己读 queue 跑
            # ────────────────────────────────────────
            types.Tool(
                name="task_submit",
                description=(
                    ...
                    "namespace 默认 'tasks'。"
                    "OIagent 调度时通过 task_list --ready 拉 ready task 跑。"
                ),
                inputSchema={
                    "type": "object",
                    "properties": {
                        "title": {"type": "string", "description": "任务标题(简短)"},
                        "content": {"type": "string", "description": "任务描述(详细,OIagent 拿到后能直接跑)"},
                        "depends_on": {
                            "type": "array",
                            "items": {"type": "integer"},
                        ...
                        "max_retries": {"type": "integer", "default": 3, "description": "OIagent 失败重试次数"},
                    },

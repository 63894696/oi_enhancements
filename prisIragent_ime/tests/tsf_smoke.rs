...
fn keystroke_buffer_echo() {
    let mut buf = prisiragent_ime::PinyinBuffer::new();
    assert_eq!(buf.buf, "");
...
fn keystroke_buffer_digit_space() {
    ...
    // 空格选首位(无候选时提交 buf)
    let mut buf2 = prisiragent_ime::PinyinBuffer::new();
    ...
    // 空格无候选时返回 buf
    let mut buf3 = prisiragent_ime::PinyinBuffer::new();
...
fn ffi_path_lookup_returns_string() {
    ...
    let p = prisiragent_ime::ffi::locate_dll_path_for_test();
...
fn ffi_sanity_load() {
    ...
    let json_ptr = unsafe { prisiragent_ime::ffi::prisir_tsf_query(h, pin.as_ptr()) };
    ...
    unsafe { prisiragent_ime::ffi::prisir_tsf_free_string(json_ptr); };
    unsafe { prisiragent_ime::ffi::prisir_tsf_free_engine(h); };
}
...
fn register_path_strips_trailing_slash() {
    use prisiragent_ime::register::normalize_dll_path;
...
fn register_idempotent_paths() {
    use prisiragent_ime::register::{build_reg_tree, normalize_dll_path};
    ...
    // 两组 9 个,实测加起来 21。
    let key_only = prisiragent_ime::register::key_only_paths();
    assert_eq!(
        key_only.len(),
        21,
    ...
    for entry in &tree1 {
        let (sub, _name) = match entry {
            prisiragent_ime::register::RegEntry::Sz(p, _) => {
                let (s, n) = prisiragent_ime::register::split_key_value(p);
                (s, n)
            }
            prisiragent_ime::register::RegEntry::Dword(p, _) => {
                let (s, n) = prisiragent_ime::register::split_key_value(p);
                (s, n)
            }
        };
...
fn daemon_path_parse_dll() {
    let c = prisiragent_ime::daemon::DaemonConfig::default();
    assert!(
...
fn register_catid_real_msctf() {
    use prisiragent_ime::register::{build_reg_tree, CATEGORY_TIP_KEYBOARD};
...
fn daemon_wts_event_filter() {
    ...
    assert!(
        !prisiragent_ime::daemon::is_shutdown(),
        "T5: SHUTDOWN 标志位默认必须 false"
    );
    // request_shutdown 后必须 true
    prisiragent_ime::daemon::request_shutdown();
    assert!(
        prisiragent_ime::daemon::is_shutdown(),
        "T5: request_shutdown 后 is_shutdown 必须 true"
    );
    // 复位,避免污染同进程后续测试
    prisiragent_ime::daemon::reset_shutdown_for_test();
    assert!(
        !prisiragent_ime::daemon::is_shutdown(),
...
fn ipc_request_parse() {
    use prisiragent_ime::ipc::handle_request;
    ...
    assert!(
        resp.contains("\"crate\":\"prisiragent_ime\""),
        "version method 必须含 crate=prisiragent_ime, 实得: {resp}"
    );
    assert!(
        resp.contains("\"id\":1"),
...
fn method_dispatch_table() {
    use prisiragent_ime::hotkey::{InputMethod, parse_method};
    ...
    assert_eq!(
        prisiragent_ime::keystroke::active_method(),
        InputMethod::Pinyin,
        "T7: 进程启动默认 active_method 必须是 Pinyin"
    );
    // 切到 Wubi 后能读到
    prisiragent_ime::keystroke::set_active_method(InputMethod::Wubi);
    assert_eq!(
        prisiragent_ime::keystroke::active_method(),
        InputMethod::Wubi,
    ...
    // 复位,避免污染同进程后续测试
    prisiragent_ime::keystroke::set_active_method(InputMethod::Pinyin);

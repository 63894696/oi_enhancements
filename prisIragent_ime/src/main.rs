...
fn main() {
    ...
    match args.get(1).map(|s| s.as_str()) {
        ...
        Some("--method") => {
            ...
            match prisiragent_ime::parse_method(arg) {
                Ok(m) => {
                    prisiragent_ime::keystroke::set_active_method(m);
                    println!("[method] active: {}", m.as_str());
                    println!("[method] trigger: {}",
                        match m {
                            prisiragent_ime::InputMethod::Pinyin => format!("Right Ctrl (0x{:02X})", prisiragent_ime::PINYIN_TRIGGER_KEY),
                            prisiragent_ime::InputMethod::Wubi   => format!("Right Shift (0x{:02X})", prisiragent_ime::WUBI_TRIGGER_KEY),
                        });
        ...
        Some("--ipc") => {
            ...
            if let Err(e) = prisiragent_ime::ipc::run_ipc_server() {
...
fn run_register(dll_override: Option<String>) {
    ...
    match prisiragent_ime::register::do_register(&dll) {
        Ok(r) => {
            println!("[register] OK ({} entries written)", r.entries_written);
            println!("[register] CLSID: {}", prisiragent_ime::register::CLSID_PRISIR_IME_STR);
            println!("[register] 重要: Windows CTF 注册后必须重启 explorer.exe 才生效");
            println!("[register] 验证: 运行 `prisir_tsfsvc --status` 或 `reg query \"HKCU\\SOFTWARE\\Microsoft\\CTF\\TIP\\{}\\;`", prisiragent_ime::register::CLSID_PRISIR_IME_STR);
            std::process::exit(0);
        }
        Err(e) => {
...
fn run_register_elevated() {
    use prisiragent_ime::elevate::{run_elevated, ElevateResult};
...
fn run_register_status() {
    match prisiragent_ime::register::read_hklm_inprocserver32_status() {
        Ok(s) => {
            let payload = serde_json::json!({
                ...
                "verdict": s.verdict,
                "clsid": prisiragent_ime::register::CLSID_PRISIR_IME_STR,
...
fn run_unregister() {
    println!("[unregister] removing HKCU CTF TIP + COM CLSID keys for CLSID={}", prisiragent_ime::register::CLSID_PRISIR_IME_STR);
    match prisiragent_ime::register::do_unregister() {
        Ok(()) => {
...
fn run_status() {
    match prisiragent_ime::register::do_status() {
        Ok(s) => {
            let active = prisiragent_ime::keystroke::active_method();
            let payload = serde_json::json!({
...
fn run_ipc_test() {
    ...
    for line in tests {
        ...
        let resp = prisiragent_ime::ipc::handle_request(line);
...
fn run_enable() {
    match prisiragent_ime::register::do_enable() {
        Ok(()) => {
...
fn run_disable() {
    match prisiragent_ime::register::do_disable() {
        Ok(()) => {
...
/// guest `C:\Temp\dumps\`。
fn run_activate() {
    match prisiragent_ime::register::do_activate() {
        Ok(()) => {
...
fn run_activate_mgr() {
    match prisiragent_ime::register::do_activate_mgr() {
        Ok(()) => {
...
fn run_enum_profiles() {
    match prisiragent_ime::register::do_enum_profiles() {
...
fn run_register_profile() {
    ...
    match prisiragent_ime::register::do_register_profile(&dll) {
        Ok(()) => {
...
fn run_daemon() {
    let config = prisiragent_ime::daemon::DaemonConfig::default();
    if let Err(e) = prisiragent_ime::daemon::run_daemon(config) {
...
fn run_query_test(pinyin: &str, db: &str) {
    ...
    #[allow(unused_unsafe)]
    let h = unsafe { prisiragent_ime::ffi::prisir_tsf_load_engine(db_c.as_ptr()) };
    if h.is_null() {
    ...
    #[allow(unused_unsafe)]
    let json_ptr = unsafe { prisiragent_ime::ffi::prisir_tsf_query(h, pinyin_c.as_ptr()) };
    if json_ptr.is_null() {
        eprintln!("[main] prisir_tsf_query 返回 null");
        prisiragent_ime::ffi::prisir_tsf_free_engine(h);
        std::process::exit(4);
    }
    let json = unsafe { std::ffi::CStr::from_ptr(json_ptr) }.to_string_lossy().into_owned();
    println!("[main] query '{}' → {}", pinyin, json);
    prisiragent_ime::ffi::prisir_tsf_free_string(json_ptr);

    let sent_in = "nihaoshijie";
    let sent_c = CString::new(sent_in).unwrap();
    #[allow(unused_unsafe)]
    let sent_ptr = unsafe { prisiragent_ime::ffi::prisir_tsf_smart_sentence(h, sent_c.as_ptr()) };
    if sent_ptr.is_null() {
        ...
        prisiragent_ime::ffi::prisir_tsf_free_string(sent_ptr);
    }

    let learn_in = CString::new("nihao").unwrap();
    let learn_sel = CString::new("你好").unwrap();
    prisiragent_ime::ffi::prisir_tsf_learn(h, learn_in.as_ptr(), learn_sel.as_ptr());

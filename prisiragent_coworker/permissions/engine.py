...
#
# Modifications by Prisiragent Project Contributors:
#   - Renamed package openworker -> prisiragent_coworker; replaced openworker audit
#     sink with prisiragent.audit.P2_10; shell op regex list extended with
#     PowerShell + cmd.exe on Windows; standing rule expiry default shortened
#     from 1h to 15min per Prisiragent P2-10 risk profile.
...
"""Prisiragent Coworker -- five-mode permission decision engine (W2-1.1).

This module is the core of Prisiragent Coworker's permission system. It replaces
Prisiragent P0-3 PolicyEngine for the five-mode decision path
...
Anti-flattery boundary (see plan §3.1 / §8.1.1):
    ...
    - Audit goes through an injected sink; callers wire this to
      `prisiragent.audit.P2_10_audit_sink`.
"""
...
# AuditSink protocol now lives in prisiragent_coworker.permissions.audit
...
#   - openai / anthropic direct SDK calls (routed via Prisiragent 15721 proxy)
...
# Default standing-rule TTL: 15 minutes (Prisiragent P2-10 risk profile).
# Upstream source project used 1h; Prisiragent tightened to 15min per plan §3.1.
    ...
    def __init__(
        ...
        Args:
            ...
            audit_sink: Callable invoked with one ``AuditDecision`` argument
                ...
                ``prisiragent.audit.P2_10_audit_sink`` via the audit facade.

...
# 或 oiagent_web.py 同级 logs/(开发态 fallback)。
_LOGGER = logging.getLogger("oiagent_web")
...
def _setup_logging(log_file: str | None = None) -> str:
    ...
    try:
        ...
        sys.stderr.write(f"[oiagent_web] log_file unreachable, fallback to {target}: {e}\n")
    try:
        ...
        sys.stderr.write(f"[oiagent_web] logging setup failed: {e}\n")
    return target
...
def _run_chat_thread(sid: str, user_text: str, strategy: str, model: str, workdir: str,
    ...
    try:
        ...
        try:
            import oiagent_cli  # noqa: PLC0415
            written = oiagent_cli.pop_written_files(workdir)
            if written:
...
#   Win 装包态:%APPDATA%/oiagent-shell/(= Electron app.getPath('userData'))
#   源码态   :同样落到 APPDATA,保持单一落点便于用户一键打包。
_USER_DATA_DIR = Path(os.environ.get(
    "APPDATA", str(Path.home()))) / "oiagent-shell"
...
// 页面内相对 fetch('/oiagent/api/...') 会 401(会话/模型/图标全空,用户实测「重开 App 又没了」)。
// 双保险:把 URL 上的 token 存进 sessionStorage,并重写 fetch 给同源 /oiagent/api 请求自动
  ...
  try{
    ...
    window.fetch=function(url, opts){
      try{
        ...
        // 只补同源 /oiagent/ 的相对或绝对请求,且未带 token 的
        if(u.indexOf("/oiagent/")===0 || u.indexOf(location.origin+"/oiagent/")===0){
    ...
    // 遍历所有指向 /oiagent/ 的 img,给 src 追加 token。cookie 持久化在部分 WebView 版本
    // 不可靠(iframe 第三方 cookie),这是图标的兜底。DOMContentLoaded + 延迟各跑一次,
    // 覆盖静态标签与后续 JS 动态插入的 img。
    var fixImgs=function(){
      try{
        document.querySelectorAll('img[src^="/oiagent/"]').forEach(function(im){
...
<div id="topbar">
  <div id="brand">
    <img src="/oiagent/assets/prisIr-flame-48.png" alt="icon">
    ...
    <div id="conv-head">
      ...
      <div id="menu-wrap">
        ...
        <div id="menu">
          ...
          <div class="mi" onclick="window.open('/oiagent/remote','_blank')" data-i18n="remote">📱 手机遥控</div>
          <div class="divider"></div>
          <div class="mi" onclick="window.open('/oiagent/about','_blank')" data-i18n="about">ℹ️ 关于</div>
          <div class="divider"></div>
...
//   点「发布到论坛」:POST /oiagent/api/feedback_zip 打 zip + 经主进程 IPC 打开论坛反馈页
...
async function feedbackBuildZip(){
  ...
  let r;
  try {
    r = await fetch('/oiagent/api/feedback_zip', {
      method: 'POST', headers: {'Content-Type':'application/json'},
...
<div class="wrap">
  <div id="brand">
    <img src="/oiagent/assets/prisIr-flame-48.png" alt="">
    <span class="name">Prisir AI</span>
  ...
  <div class="foot">
    <a href="/oiagent/about" data-l="about">关于</a>·
    <a href="/oiagent/privacy" data-l="privacy">隐私说明</a>·
    <a href="/oiagent/terms" data-l="terms">使用条款</a>·
...
function genOffer(){
  ...
  box.textContent = _ZH ? '…' : '…';
  fetch('/oiagent/api/pair/offer').then(function(r){return r.json();}).then(function(d){
...
(function(){
  var l=(navigator.language||navigator.userLanguage||'zh').toLowerCase();
  _ZH = l.startsWith('zh');
  fetch('/oiagent/api/info').then(function(r){return r.json();}).then(function(d){
...
function renderHit(h,list){
  ...
  if(h.shot){
    ...
    view.href='/oiagent/api/fcontent/shot_image?path='+encodeURIComponent(h.path);view.target='_blank';
    ...
    tr.onclick=async(ev)=>{
      ...
      try{
        ...
        view.href='/oiagent/api/fcontent/shot_image?path='+encodeURIComponent(r.out);
        const orig=document.createElement('a');orig.textContent='🖼 原图';orig.style.cssText=btnStyle;
        orig.href='/oiagent/api/fcontent/shot_image?path='+encodeURIComponent(h.path);orig.target='_blank';
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _gate(self, path: str) -> bool:
        ...
        if path == "/oiagent/api/info" and lp.is_lan_client(ip):
            return False
        # 其余远程请求必须带持久令牌(头 X-Prisir-Token、?token= qs,或配对时种下的 cookie)
        # cookie 兜底:手机 iframe 只在初始 URL 带 ?token=,页面加载后前端所有
        # fetch('/oiagent/api/...') 与 <img src=...> 都是相对路径不带 token,私网源会被 401
    ...
    def do_GET(self):  # noqa: N802
        ...
        # P2 SSE 推流:移动端连 /oiagent/events?token= 收实时工具进度/答复。
        # 令牌鉴权同 HTTP(?token= qs);纯 HTTP 长连接,handler 线程内循环写流。
        if path == "/oiagent/events":
            lp = lan_pair.instance()
        ...
        if path in ("/", "/index.html"):
        ...
        elif path == "/oiagent/api/pair/offer":
            ...
            try:
                ...
                self._json({"items": []})
        elif path == "/oiagent/api/solutions":
        ...
        elif path == "/oiagent/api/fcontent/status":
            ...
            st["ready"] = True
            self._json(st)
        elif path == "/oiagent/api/fcontent/search":
        ...
        elif path == "/oiagent/api/fcontent/shots":
            # 列截图存档(新→旧)。
            fc = _fcontent()
            if fc is None:
        ...
        elif path == "/oiagent/api/fcontent/shot_image":
    ...
    def do_POST(self):  # noqa: N802
        ...
        elif path == "/oiagent/api/pair/confirm":
        ...
        elif path == "/oiagent/api/estop":
            # 紧急停止:置中断标志 + 唤醒挂起的权限卡。前端「停止」按钮调用。
            sid = body.get("session_id", "")
            _estop_set(sid)
            self._json({"ok": True})
        elif path == "/oiagent/api/estop/clear":
            _estop_clear(body.get("session_id", ""))
            self._json({"ok": True})
        elif path == "/oiagent/api/profile/archive":
        ...
        elif path == "/oiagent/api/solutions/archive":
        ...
        elif path == "/oiagent/api/fcontent/enable":
        ...
        elif path == "/oiagent/api/fcontent/disable":
        ...
        elif path == "/oiagent/api/perm_confirm":
        ...
        elif path == "/oiagent/api/feedback_zip":
...
def main():
    ...
    ap.add_argument("--log-file", default=None,
                    help="诊断日志落点(默认 %%APPDATA%%/oiagent-shell/logs/prisirai-backend.log)")
    args = ap.parse_args()

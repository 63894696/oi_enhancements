# -*- coding: utf-8 -*-
# oiagent_dev_consumer.py — oiagent 开发团队协作链路消费者守护(2026-08-14)
#
# 背景(F1 stall 根因):派单走 task_submit → memory.task_queue.TaskQueue,
# 消费端是 mcp_oiagent_v3_plan_parallel workflow 的 task_queue_input 节点。
...
#
# 用法:
#   python oiagent_dev_consumer.py              # 前台跑(看日志)
#   python oiagent_dev_consumer.py --workers 4  # 4 并发
#   python oiagent_dev_consumer.py --daemon     # 后台分离跑(无窗)
#   python oiagent_dev_consumer.py --install    # 注册开机自启(登录触发)
#
# 红线:
...
LOG_FILE = LOG_DIR / "oiagent_dev_consumer.log"
TASK_NAME = "OIAgentDevConsumer"
LOCK_FILE = LOG_DIR / "oiagent_dev_consumer.lock"

logging.basicConfig(
    level=logging.INFO,
    format="[%(asctime)s] %(levelname)s %(name)s: %(message)s",
)
log = logging.getLogger("oiagent.dev_consumer")
...
def _build_system() -> str:
    base = (
        "你是 oiagent 开发协作团队的执行 agent。下面先给你【项目宪法】——硬性技术契约,"
    ...
    try:
        ...
        pat = _dp.recall_patterns("oiagent 开发 实现 方案", role="consumer", n=3)
        if pat:
...
def run_one(worker_idx: int) -> bool:
    ...
    from mcp_oiagent_v3_plan_parallel.workflow import (  # noqa: PLC0415
        LimitsConfig, WorkflowEntry,
    )
    from mcp_oiagent_v3_plan_parallel.workflow.events import (  # noqa: PLC0415
...
def run_forever(workers: int, idle_sleep: float) -> None:
    ...
    if not _verify_kernel():
        ...
        return
    log.info("oiagent dev-consumer 启动 workers=%d idle_sleep=%.0fs (内核校验通过)", workers, idle_sleep)
    threads = [

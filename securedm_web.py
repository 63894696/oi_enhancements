    ...
    _this.createTableTag = function(cellSize, margin) {
      ...
      for (var r = 0; r < _this.getModuleCount(); r += 1) {
        ...
        for (var c = 0; c < _this.getModuleCount(); c += 1) {
          ...
          qrHtml += _this.isDark(r, c)? '#12395b' : '#ffffff';
          qrHtml += ';';
          qrHtml += '"/>';
        }
...
def _db_migrate_plaintext_to_encrypted(key_hex: str) -> dict[str, Any]:
    """把【已存在的明文 simplex 库】原地迁移为 SQLCipher 加密库(sqlcipher_export)。

    背景:用户可能先在明文库里聊过、再设口令。若不迁移,库里已有明文数据 + 又写了
    key 文件,下次解锁时 libsimplex 拿密钥去开明文库 → ChatInitError(任务 #6 的坑)。
    此处对已存在的明文 *_agent.db / *_chat.db 逐个做 sqlcipher_export 原地加密。
    明文备份仅在【迁移途中失败】时用于回滚;全部迁移成功后立即删除,不留明文副本
    (避免明文落盘被窃/被杀软误报)。回滚窗口 = 迁移期间;迁完即不可回滚(已加密)。

    返回 {"migrated": [文件名...]};无明文库可迁时 migrated 为空列表(正常)。
    任一库迁移失败立即返回 _err,且已迁的库会被回滚成明文,保证不半成品。
    """
    import shutil
    import time
    ...
    targets = []
    for f in sorted(parent.glob(base + "_*.db")):
        try:
            if open(f, "rb").read(8).startswith(b"SQLite"):
                targets.append(f)  # 仅迁明文库;已是加密库的跳过
        except Exception:  # noqa: BLE001
            pass
    if not targets:
        return {"migrated": []}
    ...
    migrated: list[str] = []
    done: list[tuple[Path, Path]] = []  # (原库, 明文备份) 供回滚
    try:
        for f in targets:
            enc = f.with_name(f.name + ".enc")
            bak = f.with_name(f.name + f".pre-encrypt-{ts}")
            conn = _sqlcipher.connect(str(f))          # 明文库,无 key
            # simplex 用 PRAGMA key='<hex>'(passphrase 形式),sqlcipher_export 保持同参
            enc_fwd = str(enc).replace(os.sep, "/")
            conn.execute("ATTACH DATABASE '%s' AS encrypted KEY '%s'" % (enc_fwd, key_hex))
            conn.execute("SELECT sqlcipher_export('encrypted')")
            conn.execute("DETACH DATABASE encrypted")
            conn.close()
            shutil.move(str(f), str(bak))              # 明文 → 备份
            shutil.move(str(enc), str(f))              # 加密库 → 原位
            done.append((f, bak))
            migrated.append(f.name)
    except Exception as e:  # noqa: BLE001
        # 回滚:已替换的库恢复成明文备份,删半成品 enc
        for orig, bak in done:
            try:
                if bak.exists():
        ...
        for f in targets:
            try:
                ...
                pass
        return _err("明文库加密迁移失败", f"{e!r}。已回滚为明文库,口令未生效;可重试或检查磁盘/权限。")
    # 全部迁移成功:立即删除明文备份,不留明文副本(防窃取/防杀软误报)。
    # 删除失败仅记诊断,不影响已完成的加密迁移。
    for _orig, bak in done:
        try:
            if bak.exists():
                bak.unlink()
        except Exception:  # noqa: BLE001
...
def api_db_set_password(password: str) -> dict[str, Any]:
    ...
    对已存在的明文库,先用 sqlcipher_export 原地迁移为加密库(见 _db_migrate_*),
    避免"明文库 + key 文件 → 解锁 ChatInitError"的不一致(任务 #6/#8)。"""
    import hashlib
    import secrets
    ...
    key_hex = key.hex()
    # 先迁移已存在的明文库(失败则整体中止,不落 key 文件,保持明文可用状态)
    mig = _db_migrate_plaintext_to_encrypted(key_hex)
    if isinstance(mig, dict) and mig.get("ok") is False:
        return mig
    kp.parent.mkdir(parents=True, exist_ok=True)
    kp.write_text(salt.hex() + ":" + key_hex, encoding="utf-8")
    try:
        os.chmod(kp, 0o600)
    except Exception:  # noqa: BLE001
        pass
    _rt()._db_key = key_hex  # 内存注入(不落 env);下次 _boot 以加密库打开
    moved = mig.get("migrated") if isinstance(mig, dict) else []
    extra = ("已把现有明文聊天记录加密迁移(%s)。" % "、".join(moved)) if moved else ""
    return _ok({"encrypted": True, "migrated": moved}, diagnosable="口令已设置。" + extra + "重启后会要求输入口令解锁聊天记录;请务必记牢,丢失无法恢复。")
...
<style>
  :root{--bg:#f6f1e7;--panel:#fbf8f1;--line:#d8cfbc;--txt:#2f3a34;--dim:#8a968e;--acc:#4a5c52;--ok:#3fb27f;--warn:#e0a34a;--bad:#b23a30;--gold:#d6b26c;}
  *{box-sizing:border-box}
  body{margin:0;background:var(--bg) url('/dm/assets/guohua_bg_panel.png') center bottom/cover fixed no-repeat;color:var(--txt);font:15px/1.5 -apple-system,"Segoe UI",Roboto,"Microsoft YaHei",sans-serif;height:100vh;display:flex;flex-direction:column}
  header{padding:11px 16px;border-bottom:1px solid var(--line);display:flex;align-items:center;gap:10px;background:rgba(251,248,241,.85);backdrop-filter:blur(6px)}
  header .dot{width:9px;height:9px;border-radius:50%;background:var(--ok)}
  header h1{font-size:16px;margin:0;font-weight:600;color:var(--acc)}
  header .st{margin-left:auto;font-size:12px;color:var(--dim)}
  #main{flex:1;display:flex;min-height:0}
  #contacts{width:250px;border-right:1px solid var(--line);overflow-y:auto;display:flex;flex-direction:column;background:rgba(239,232,218,.5)}
  ...
  #contacts .hd button{background:var(--acc);border:none;color:#fbf6ec;padding:5px 10px;border-radius:7px;cursor:pointer;font-size:12px}
  #contacts .hd2{padding:8px 10px;border-bottom:1px solid var(--line);display:flex;gap:6px;flex-wrap:wrap}
  #contacts .hd2 button{background:var(--panel);border:1px solid var(--line);color:var(--txt);padding:6px 9px;border-radius:7px;cursor:pointer;font-size:12px;flex:1;min-width:64px;white-space:nowrap}
  #contacts .hd2 button:hover{border-color:var(--acc);color:var(--acc)}
  .citem{padding:11px 12px;border-bottom:1px solid var(--line);cursor:pointer}
  .citem:hover{background:#efe8da}
  .citem.active{background:#fbf8f1;box-shadow:inset 2px 0 0 var(--acc)}
  ...
  .m{max-width:78%;padding:9px 13px;font-size:15px;line-height:1.4;white-space:pre-wrap;word-break:break-word;box-shadow:0 1px 3px rgba(74,92,82,.15)}
  /* 我:右侧,印章红,右下角收平(LTR 发送方侧) */
  .m.me{align-self:flex-end;background:linear-gradient(135deg,#b23a30,#9c3229);color:#fbf6ec;border-radius:18px 18px 4px 18px}
  /* 对方:左侧,宣纸面 + 墨绿左边框,左下角收平 */
  .m.them{align-self:flex-start;background:#fbf8f1;color:#2f3a34;border:1px solid var(--line);border-left:3px solid var(--acc);border-radius:18px 18px 18px 4px}
  /* 系统/签名清单:居中、灰小字、无气泡感 */
  .m.manifest{align-self:center;background:rgba(74,92,82,.06);border:1px dashed var(--line);font-size:12px;color:var(--dim);border-radius:8px;max-width:88%}
  .vbadge{display:inline-flex;align-items:center;gap:5px;font-size:12px;padding:3px 9px;border-radius:11px;margin-top:5px}
  .vbadge.ok{background:#e3efe6;color:var(--ok);border:1px solid var(--ok)}
  .vbadge.bad{background:#f3e0de;color:var(--bad);border:1px solid var(--bad)}
  .vbadge.pending{background:var(--panel);color:var(--dim)}
  .fmsg{align-self:flex-start;background:var(--panel);padding:10px 12px;border-radius:11px;border:1px solid var(--line)}
  .fmsg .fname{font-weight:600}
  .fmsg button{background:var(--acc);border:none;color:#fbf6ec;padding:5px 12px;border-radius:7px;cursor:pointer;font-size:12px;margin-top:6px}
  .sys{align-self:center;font-size:12px;color:var(--dim)}
  #composer{display:flex;gap:9px;padding:13px 16px;border-top:1px solid var(--line);background:rgba(251,248,241,.85)}
  #in{flex:1;background:var(--panel);border:1px solid var(--line);color:var(--txt);border-radius:9px;padding:10px 12px;outline:none}
  #in:focus{border-color:var(--acc)}
  #composer button{background:var(--acc);border:none;color:#fbf6ec;padding:0 17px;border-radius:9px;cursor:pointer;font-weight:600}
  #empty{flex:1;display:flex;align-items:center;justify-content:center;color:var(--dim);flex-direction:column;gap:10px}
  #inviteModal,#acceptModal,#settingsModal{position:fixed;inset:0;background:rgba(47,58,52,.45);display:none;align-items:center;justify-content:center}
  #inviteModal .box,#acceptModal .box,#settingsModal .box{background:var(--panel);padding:20px;border-radius:12px;max-width:560px;width:90%;border:1px solid var(--line)}
  #inviteModal textarea,#acceptModal textarea{width:100%;height:90px;background:var(--bg);color:var(--txt);border:1px solid var(--line);border-radius:8px;padding:8px;font-size:12px}
  #inviteModal button,#acceptModal button{margin-top:10px;background:var(--acc);border:none;color:#fbf6ec;padding:7px 14px;border-radius:8px;cursor:pointer}
</style>
</head>
<body>
<header><span class="dot" id="livedot"></span><h1>SecureDM 加密私信</h1><span class="st" id="status">…</span></header>
<div id="compatBanner" style="background:#fff8e6;border-bottom:1px solid #e6c869;color:#7a5a12;font-size:12.5px;padding:6px 12px;display:flex;align-items:center;gap:10px">
  <span style="flex:1">⚠ 与 <b>SimpleX 官方客户端</b> 目前仅互通<b>文字消息</b>;语音/视频/屏幕共享/文件互传/群聊需在<b>本应用两端</b>之间进行。请对方也用本应用(分享本程序)。</span>
  <span onclick="document.getElementById('compatBanner').style.display='none'" style="cursor:pointer;color:#a08844;padding:0 4px" title="关闭提示">✕</span>
</div>
<div id="main">
  <div id="contacts">
  ...
  <div id="chat">
    ...
    <div id="callPanel" style="display:none;padding:10px 14px;border-top:1px solid var(--line);background:#e8f4fd">
      <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap">
        <video id="remoteVideo" autoplay playsinline style="width:280px;background:#e8f4fd;border:1px solid #b3d7f5;border-radius:8px"></video>
        <video id="localVideo" autoplay playsinline muted style="width:120px;background:#e8f4fd;border:1px solid #b3d7f5;border-radius:8px"></video>
        <div style="display:flex;flex-direction:column;gap:7px">
          <div id="callStatus" style="font-size:13px;color:var(--dim)">呼叫中…</div>
          <div style="display:flex;gap:7px">
            <button onclick="toggleMic()" id="micBtn" style="background:#ffffff;border:1px solid #b3d7f5;color:#12395b;padding:6px 12px;border-radius:7px;cursor:pointer">🎤 静音</button>
            <button onclick="toggleCam()" id="camBtn" style="background:#ffffff;border:1px solid #b3d7f5;color:#12395b;padding:6px 12px;border-radius:7px;cursor:pointer">📷 关视频</button>
            <button onclick="switchCamera()" id="switchCamBtn" title="切换前后摄像头" style="background:#ffffff;border:1px solid #b3d7f5;color:#12395b;padding:6px 12px;border-radius:7px;cursor:pointer">🔄 切换摄像头</button>
            <button onclick="shareScreen()" id="screenBtn" style="background:#ffffff;border:1px solid #b3d7f5;color:#12395b;padding:6px 12px;border-radius:7px;cursor:pointer">🖥️ 共享屏幕</button>
    ...
    <div id="attach" style="display:none;padding:9px 16px;border-top:1px solid var(--line);gap:8px;align-items:center;flex-wrap:wrap">
      ...
      <input id="fpath" placeholder="文件绝对路径(或点「选文件」)" style="flex:1;min-width:180px;background:#ffffff;border:1px solid #b3d7f5;color:var(--txt);border-radius:8px;padding:8px 10px">
      ...
      <div style="width:100%;display:flex;align-items:center;gap:6px;font-size:12px;color:var(--dim)">
        ...
        <button onclick="pickDownloadDir()" title="自定义下载保存目录(桌面壳)" style="background:#ffffff;border:1px solid #b3d7f5;color:var(--dim);padding:3px 9px;border-radius:6px;cursor:pointer;font-size:12px">改目录</button>
      </div>
    </div>
  </div>
</div>
...
<div id="settingsModal"><div class="box"><b>设置</b>
  ...
  <div id="dbSetRow" style="display:flex;flex-direction:column;gap:8px">
    <input id="dbPwd" type="password" placeholder="设置口令(≥4 位)" style="background:#ffffff;border:1px solid #b3d7f5;color:var(--txt);border-radius:8px;padding:8px;font-size:13px">
    <input id="dbPwd2" type="password" placeholder="再输一遍确认(防首次打错)" style="background:#ffffff;border:1px solid #b3d7f5;color:var(--txt);border-radius:8px;padding:8px;font-size:13px">
    <div><button onclick="setDbPassword()">设置口令</button></div>
  </div>
  <div id="dbUnlockRow" style="display:none;flex-direction:column;gap:8px">
    <div style="display:flex;gap:8px"><input id="dbPwdUnlock" type="password" placeholder="输入口令解锁" style="flex:1;background:#ffffff;border:1px solid #b3d7f5;color:var(--txt);border-radius:8px;padding:8px;font-size:13px"><button onclick="unlockDb()">解锁</button></div>
    ...
    <input id="dbPwdOld" type="password" placeholder="旧口令" style="background:#ffffff;border:1px solid #b3d7f5;color:var(--txt);border-radius:8px;padding:8px;font-size:13px">
    <input id="dbPwdNew" type="password" placeholder="新口令(≥4 位)" style="background:#ffffff;border:1px solid #b3d7f5;color:var(--txt);border-radius:8px;padding:8px;font-size:13px">
    <input id="dbPwdNew2" type="password" placeholder="再输一遍新口令" style="background:#ffffff;border:1px solid #b3d7f5;color:var(--txt);border-radius:8px;padding:8px;font-size:13px">
  ...
  <div id="tfaOffRow" style="display:none;flex-direction:column;gap:8px">
    ...
    <div id="tfaQrWrap" style="display:none;flex-direction:column;gap:8px">
      ...
      <div style="display:flex;gap:8px"><input id="tfaCodeEnable" placeholder="输入 App 显示的 6 位码" maxlength="6" style="flex:1;background:#ffffff;border:1px solid #b3d7f5;color:var(--txt);border-radius:8px;padding:8px;font-size:13px"><button onclick="tfaEnable()">启用</button></div>
    </div>
  </div>
  <div id="tfaOnRow" style="display:none;flex-direction:column;gap:8px">
    <div style="font-size:12px;color:var(--ok)">已启用:解锁需口令 + 动态码</div>
    <div style="display:flex;gap:8px"><input id="tfaCodeDisable" placeholder="输入 6 位码以关闭" maxlength="6" style="flex:1;background:#ffffff;border:1px solid #b3d7f5;color:var(--txt);border-radius:8px;padding:8px;font-size:13px"><button onclick="tfaDisable()" style="background:#dceefb;color:#12395b">关闭 2FA</button></div>
  </div>
  <div id="tfaMsg" style="font-size:12px;color:var(--dim);margin-top:4px"></div>
  <div style="margin-top:16px;text-align:right"><button onclick="closeSettings()" style="background:#dceefb;color:#12395b">关闭</button></div>
</div></div>
<div id="lockGate" style="display:none;position:fixed;inset:0;background:var(--bg);z-index:999;align-items:center;justify-content:center">
  <div class="box" style="max-width:400px;width:92%">
    ...
    <div style="display:flex;flex-direction:column;gap:8px">
      <input id="gatePwd" type="password" placeholder="口令" onkeydown="if(event.key==='Enter')gateUnlock()" style="background:#ffffff;border:1px solid #b3d7f5;color:var(--txt);border-radius:8px;padding:10px;font-size:14px">
      <input id="gateTotp" placeholder="2FA 动态码(如已启用)" maxlength="6" onkeydown="if(event.key==='Enter')gateUnlock()" style="display:none;background:#ffffff;border:1px solid #b3d7f5;color:var(--txt);border-radius:8px;padding:10px;font-size:14px">
...
<div id="firstRunGate" style="display:none;position:fixed;inset:0;background:var(--bg);z-index:999;align-items:center;justify-content:center">
  <div class="box" style="max-width:420px;width:92%">
    ...
    <div style="display:flex;flex-direction:column;gap:8px">
      <input id="frPwd" type="password" placeholder="设置口令(≥4 位)" style="background:#ffffff;border:1px solid #b3d7f5;color:var(--txt);border-radius:8px;padding:10px;font-size:14px">
      <input id="frPwd2" type="password" placeholder="再输一遍确认" style="background:#ffffff;border:1px solid #b3d7f5;color:var(--txt);border-radius:8px;padding:10px;font-size:14px">
      <div style="display:flex;gap:8px"><button onclick="frSetPassword()" style="flex:1;padding:10px">设口令并加密</button><button onclick="frSkip()" style="background:#dceefb;color:#12395b;padding:10px">跳过(明文)</button></div>
...
async function openSettings(){
  ...
  // 「输入口令解锁」入口已由 bootGate 密码门接管,settings 里只对真正 needs_unlock=true 的会话显示,
  // 避免已 unlock 进 settings 后还看到冗余输入框造成误导。
  const lr=await api('/dm/api/db_lock_status',{method:'POST',body:'{}'});
  const needsUnlock=lr.ok&&lr.output.needs_unlock;
  document.getElementById('dbUnlockRow').style.display=needsUnlock?'flex':'none';
  tfaStatusRefresh();
}
...
function _showScreenView(ev){
  ...
  if(!wrap){
    ...
    wrap.style.cssText='position:fixed;top:6%;left:50%;transform:translateX(-50%);width:64vw;height:70vh;background:#e8f4fd;border:2px solid var(--acc);border-radius:10px;z-index:1000;overflow:hidden;box-shadow:0 8px 40px rgba(0,0,0,.6)';
    // 标题栏(可拖动)
    ...
    sv.style.cssText='width:100%;height:calc(100% - 32px);object-fit:contain;background:#e8f4fd';
    wrap.appendChild(sv);
    // 右下角缩放手柄
...
function renderTotpQr(text){
  ...
  try{
    ...
    ctx.fillStyle='#ffffff';ctx.fillRect(0,0,200,200);ctx.fillStyle='#12395b';
...
class DMHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path.startswith("/dm/assets/"):
            return self._asset(path[len("/dm/assets/"):])
        if path in ("/", "/dm"):
            if not self._auth():
